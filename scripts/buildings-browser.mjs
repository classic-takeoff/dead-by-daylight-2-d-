...
assert.notEqual(snapshots[0].seed,snapshots[1].seed);assert.notDeepEqual(snapshots[0].terrain,snapshots[1].terrain);assert.match(await page.locator('.map-title small').innerText(),new RegExp(snapshots[1].label));await page.locator('#pause-btn').click();assert.match(await page.locator('#panel').innerText(),new RegExp(snapshots[1].label));assert.deepEqual(snapshots[0].gates,snapshots[1].gates);assert.equal(snapshots[0].pallets,23);await page.getByRole('button',{name:'继续对局',exact:true}).click();for(const area of ['shack','upstairs','basement']){await page.evaluate(area=>{const f=window.fogbound,m=f.match;f.scene.paused=true;m.player.transition=0;if(area==='shack')Object.assign(m.player,{x:480,y:190,level:0});else if(area==='upstairs')Object.assign(m.player,{x:920,y:550,level:1});else Object.assign(m.player,{x:m.basement.room.x+m.basement.room.w/2,y:m.basement.room.y+m.basement.room.h/2,level:-1});},area);await page.waitForTimeout(100);await page.screenshot({path:'artifacts/building-'+area+'.png'});}assert.deepEqual(errors,[]);console.log({mapsDiffer:true,seeds:snapshots.map(s=>s.label),gatesFixed:true,pallets:23,errors});
