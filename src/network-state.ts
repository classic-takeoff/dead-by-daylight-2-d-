...
export function applySnapshot(m:Match,text:string){
 ...
 const previous=m.actors;const positions=m.actors.map(actorPosition);for(const key of fields)if(key in s)(m as unknown as Record<string,unknown>)[key]=s[key];
 m.visualSync={from:Math.min(from,m.time),at:now,duration};
 m.actors.forEach((a,i)=>{const old=previous[i],reset=old.life!==a.life||old.lockerId!==a.lockerId||old.motionRevision!==a.motionRevision||Math.hypot(old.x-a.x,old.y-a.y)>TUNING.network.snapDistance;a.netFrom=reset?undefined:positions[i];a.netAt=now;a.netDuration=duration;});
 ...
 m.actors.forEach(a=>{a.netExtrapolate=remoteEndpoint(m,a);});
 m.events.push(...(s.events??[]));
}
export async function pack(text:string){
 const bytes=zlibSync(strToU8(text),{level:1});
