import {ClientMotionAuthority} from './client-motion';
...
export class Multiplayer{
 ...
 private prediction=new ClientPrediction();private motion=new ClientMotionAuthority();
 ...
 private begin(seed:number){
  this.round=seed;this.motion=new ClientMotionAuthority();this.remoteInputs.clear();this.received=-1;this.sequence=0;this.prediction=new ClientPrediction();
 ...
 private async handle(e:any){
  ...
  if(e.op==='member_left'){
   ...
   if(this.isHost){const s=this.slots.find(s=>s.clientId===e.member.client_id);if(s){s.clientId=null;s.ai=this.started;s.name=this.started?'AI 接管':'空位';this.remoteInputs.delete(s.id);this.motion.forget(s.id);this.shareLobby();}}return;
  }
  ...
  if(this.isHost){
   ...
   if(d.type==='hello'){
    ...
    const input=cleanInput(d.input);this.motion.accept(this.match,s.id,input,d.motion);if(old){input.space||=old.input.space;input.attack||=old.input.attack;input.special||=old.input.special;input.taunt??=old.input.taunt;}this.remoteInputs.set(s.id,{input,at:performance.now(),seq:d.seq});}
   return;
  }
  ...
  if(d.type==='state'&&this.match&&d.round===this.round&&Number.isSafeInteger(d.seq)&&d.seq>this.received){const current=this.match;const text=await unpack(d.payload);if(d.seq<=this.received||this.closed||this.match!==current)return;this.received=d.seq;const previous=actorPosition(current.player);applySnapshot(current,text);this.prediction.reconcile(current,Number.isSafeInteger(d.acks?.[current.playerId])?d.acks[current.playerId]:-1,previous);}
 }
 tick(dt:number,input:Input,fromTimer=false){
  ...
  if(this.isHost){
   ...
   const before=m.actors.map(a=>({x:a.x,y:a.y,level:a.level,life:a.life,locker:a.lockerId,transition:a.transition,lunge:!!a.lunge}));
   m.humanInputs=controls;m.step(dt,input);
   for(const id of m.clientMoved){const a=m.actors[id],b=before[id];if(a.x!==b.x||a.y!==b.y||a.level!==b.level||a.life!==b.life||a.lockerId!==b.locker||a.transition>b.transition||!!a.lunge!==b.lunge){a.hostView=undefined;a.motionRevision=(a.motionRevision??0)+1;this.motion.forget(id);}}
   ...
   this.prediction.tick(m,dt,input);
  ...
  if(!this.isHost){const seq=++this.sequence;this.prediction.sent(seq);this.direct(this.hostId,{type:'input',round:this.round,seq,input:this.pending,motion:this.prediction.report(m)});this.pending={...this.pending,space:false,special:false,attack:false,taunt:undefined};}
  else if(!this.packing){this.packing=true;const events=m.events;const queued=this.eventQueue.splice(0).slice(-16);if(fast&&queued.length)this.broadcast({type:'round-events',round:this.round,events:queued});m.events=fast?[]:queued;const text=snapshot(m);m.events=events;const seq=++this.sequence,round=this.round,acks=m.actors.map(a=>this.remoteInputs.get(a.id)?.seq??-1);void pack(text).then(payload=>{if(this.match!==m||this.closed)return;if(payload.length>7700)throw Error('联机状态超过房间包大小');
   // A frame containing relay events must remain on the reliable relay even
   // if ICE finishes connecting while compression is in progress.
   if(fast)this.broadcast({type:'state',round,seq,acks,payload});else this.send({op:'broadcast',data:{v:PROTOCOL,type:'state',round,seq,acks,payload}});
